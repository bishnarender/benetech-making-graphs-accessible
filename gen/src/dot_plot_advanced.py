...
try:
    ...
    from generator_utils import is_numeric
except ImportError:
...
def generate_random_params():
    ...
    if p <= 0.5:
        ...
        lift_factor = random.uniform(0.5, 0.9)
    ...
    params = {
        ...
        'aux_spine': random.random() >= 0.9,
        'left_spine': random.random() >= 0.5,


        'custom_spine': random.random() >= 0.98,
...
def adjust_params(params):
    ...
    h_factor = random.uniform(3.0, 4.0)
    if params['max_count'] >= 12:
        h_factor = random.uniform(3.0, 3.5)

    height = max(2, params['max_count'] / h_factor)
    ...
    elif (params['max_x_tick_chars'] <= 5) & (params['n_points'] <= 8):
        params['xtick_label_rotation'] = 0
        params['font_size'] = max(8, params['font_size'])
...
class BasicDotPlot:
    ...
    def set_axis_limits(self):
        ...
        if not is_numeric(self.example['x_series']):
            ...
            x_lb -= random.uniform(0.1, 0.5)
            x_ub += random.uniform(0.1, 0.5)
        ...
        if p <= 0.25:
            ...
            y_lb = random.uniform(-1.0, -0.5)
            y_ub = max(self.y_values) + random.randint(0, 4)
    ...
    def configure_ticks(self):
        ...
        if not is_numeric(self.example['x_series']):
            ...
            x_tick_labels = ["\n".join(textwrap.wrap(c, width=8)) for c in self.x_values]
    ...
    def configure_titles(self):
        ...
        axes_font_dict['size'] = self.params['font_size']
        ...
        x_title = self.title_dict['x_title']
        x_title = '\n'.join(textwrap.wrap(x_title, 8))
        p = random.random()
        if p <= 0.7:
            self.ax.set_xlabel(
    ...
    def make_basic_plot(self, x, y):
        ...
        # print(self.params['marker'])

        for xi, yi in zip(x, y):
            self.ax.plot(
                [xi]*yi,
                [i + factor for i in (range(yi))],
