...
class HorizontalBarPlot:
    ...
    def configure_ticks(self):
        ...
        y_tick_labels = [
            "\n".join(textwrap.wrap(cat, width=8)) for cat in self.y_values
        ]
        ...
        if random.random() >= 0.5:
            ...
            major_tick_locations = np.arange(-1, num_y) + 0.5
            self.ax.set_yticks(major_tick_locations, minor=False)
            self.ax.set_yticklabels(['']*len(major_tick_locations), minor=False)

            minor_tick_locations = np.arange(num_y)
            self.ax.set_yticks(minor_tick_locations, minor=True)
            self.ax.set_yticklabels(y_tick_labels, minor=True)
