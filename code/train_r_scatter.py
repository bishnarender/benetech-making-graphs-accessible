...
try:
    from r_scatter.mga_dataloader import MGACollator
    from r_scatter.mga_dataset import (TOKEN_MAP, MGADataset,
                                       create_train_transforms)
    from r_scatter.mga_model import AWP, MGAModel
...
@hydra.main(version_base=None, config_path="../conf/r_scatter", config_name="conf_r_scatter")
def run_training(cfg):
    ...
    scatter_ids = label_df["original_id"].unique().tolist()
    ...
    train_ids = list(set(train_ids).intersection(set(scatter_ids)))
    print(f'# images in original train (only scatter): {len(train_ids)}')

    extracted_ids = list(set(extracted_ids).intersection(set(scatter_ids)))
    print(f'# images in extracted (only scatter): {len(extracted_ids)}')
    ...
    valid_ids = list(set(valid_ids).intersection(set(scatter_ids)))
    ...
    if cfg.add_syn:
        ...
        # fid => ./datasets/processed/synthetic/images/syn_scatter_uvbh7vud.jpg
        # fid.split("/") => ['.', 'datasets', 'processed', 'synthetic', 'images', 'syn_scatter_uvbh7vud.jpg']
        
        syn_ids = [fid.split("/")[-1].split(".jpg")[0] for fid in syn_ids]                
        syn_ids = [gid.split("_v0")[0] for gid in syn_ids]
        
        # looking for scatter plots in synthetic plots.
        syn_ids = [gid for gid in syn_ids if 'scatter' in gid]
        ...
        print('sampling 30k syn ids')
        syn_ids = random.sample(syn_ids, k=30000)
    ...
    if cfg.add_pl:
        ...
        # looking for scatter plots in synthetic plots.
        syn_ids = [gid for gid in pl_ids if 'scatter' in gid]
    ...
    if cfg.add_icdar:
        ...
        # looking for scatter plots in icdar plots.
        icdar_ids = [gid for gid in icdar_ids if 'scatter' in gid]
