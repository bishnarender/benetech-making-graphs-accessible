...
class EMA():
    ...
    def __init__(self, model, decay):
        self.model = model
        # the decay factor used to control how much the new average depends on the current parameter values.
        self.decay = decay
        self.shadow = {}
        self.backup = {}
    ...
    def register(self):
        '''
        creates a "shadow" copy of the model's parameters by cloning the current parameter values into the self.shadow dictionary.
        '''
    ...
    def update(self):
        '''
        calculates the new average by applying exponential moving average to each parameter.        
        '''
        for name, param in self.model.named_parameters():
            if param.requires_grad:
    ...
    def apply_shadow(self):
        '''
        replace the original parameters with their "shadow" counterparts.
        '''
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                assert name in self.shadow
