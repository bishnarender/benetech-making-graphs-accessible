...
def run_sanity_check(cfg, batch, tokenizer, prefix="mga", num_examples=8):
    '''
        saves images after the dataloader step to check everything is correctly flowing. 
    '''
    ...
    for i in range(num_examples):
        ...
        text = tokenizer.decode(batch['decoder_input_ids'][i], skip_special_tokens=True)
...
def execution_setup(cfg):
    ...
    # cfg.fold => provided via command line.
    if cfg.all_data:
...
# hydra has the ability to dynamically create a hierarchical configuration by composition and override it through config files and the command line. -
# - The name Hydra comes from its ability to run multiple similar jobs - much like a Hydra with multiple heads.

# ipykernel_launcher.py: error: unrecognized arguments: -f => run this file from command line
...
def run_training(cfg):    
    ...
    extracted_ids = fold_df[(fold_df["kfold"] != 99) & fold_df["kfold"].isin(cfg.train_folds)]["id"].unique().tolist()
    ...
    train_ids = train_ids * cfg.original_multiplier # [1,2,3]*3 => [1, 2, 3, 1, 2, 3, 1, 2, 3]
    ...
    print(f'# images in "original train" after extracted (multiplier to extracted): {len(train_ids)}')
    ...
    # 0  8941f843bb04_x  generated  ...        dot  categorical
    # 1  8941f843bb04_y  generated  ...        dot    numerical
    # 2  7dbdb91fa4a7_x  generated  ...    scatter    numerical
    
    # label_df.columns => Index(['id', 'source', 'data_series', 'chart_type', 'data_type'], dtype='object')
    ...
    print(f"# of tokens in tokenizer: {cfg.model.len_tokenizer}")    
    ...
    # type(collate_fn.tokenizer) => <class 'transformers.models.t5.tokenization_t5_fast.T5TokenizerFast'>
...
#             draw_graph(model, input_data = [batch["flattened_patches"],batch["attention_mask"], batch["labels"]], expand_nested=True, save_graph=True).visual_graph            
            
            loss, loss_dict = model(
                flattened_patches=batch["flattened_patches"], # torch.Size([1, 2048, 770])
                attention_mask=batch["attention_mask"], # torch.Size([1, 2048])
                labels=batch["labels"],# torch.Size([1, 133]) => different for different elements.
            )
            ...
            if (epoch_progress + 1) % cfg.train_params.eval_frequency == 0:
                ...
                # wnwraps the model from the additional layer possible added by prepare(). 
