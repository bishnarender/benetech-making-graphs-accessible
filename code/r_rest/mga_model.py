...
class AWP:
    ...
    def __init__(self, model, optimizer, adv_param="weight", adv_lr=1, adv_eps=0.0001):
        ...
        # A string specifying the parameter name in the model where adversarial perturbation will be applied. It represents the weights that will be perturbed.
        self.adv_param = adv_param
        # The learning rate used to control the amount of perturbation applied to the weights during the adversarial attack step.
        self.adv_lr = adv_lr
        # The epsilon value used to calculate the range of perturbation for each weight during the adversarial attack step.
        self.adv_eps = adv_eps        
        self.backup = {}
        # A dictionary to store the epsilon range for each weight during the adversarial attack.        
        self.backup_eps = {}

    def attack_backward(self, batch, accelerator):
        '''
        performs the adversarial attack during the backward pass of the model's training.
        calculates the adversarial loss and applies gradients to the model's weights, after applying the adversarial perturbation.
        '''
        if self.adv_lr == 0:
            return
        self._save()
    ...
    def _attack_step(self):
        '''
        It computes the adversarial perturbation for each weight using the gradients and then applies the perturbation to the weights.        
        '''
        e = 1e-6
    ...
    def _save(self):
        '''
        save a backup of the model's weights before applying adversarial perturbation.        
        '''
        for name, param in self.model.named_parameters():
            if param.requires_grad and param.grad is not None and self.adv_param in name:
    ...
    def _restore(self,):
        '''
        restore the original weights of the model from the backup after the adversarial attack step. It resets the backup and backup_eps attributes to empty dictionaries.        
        '''
        for name, param in self.model.named_parameters():
